...
module {
   ...
   public type SystemParams = {
    ...
    public func accounts_fromArray(arr: [Account]) : Trie.Trie<Principal, Token> {
        var s = Trie.empty<Principal, Token>();
      ...
      public func proposals_fromArray(arr: [Proposal]) : Trie.Trie<Nat, Proposal> {
        ...
        for (proposal in arr.vals()) {
              s := Trie.put(s, proposal_key(personal.id), Nat.equal, proposal).0;
        
        };
        s // return s;
      };

      public let oneToken = { amount_e8s = 10_00_000 };
